...
def get_products():
    ...
    Query params:
      ...
      ?cat=Serums             — filter by category (case-insensitive)
      ?q=velvet               — search name, cat, desc, tag
      ?sort=price|name|created_at   — sort field
    ...
    if q:
        products = [
            p for p in products
            if any(q in str(p.get(field, '')).lower() for field in ['name', 'cat', 'desc', 'tag'])
        ]

    # ── Sort ──
    sort_field = request.args.get('sort', 'created_at')
    sort_order = request.args.get('order', 'asc').lower()
    valid_sort_fields = {'price', 'name', 'created_at', 'stock'}
    ...
    try:
        ...
        limit = min(100, max(1, int(request.args.get('limit', total))))  # default: all
    except ValueError:
...
def update_product(product_id):
    """PUT /api/products/:id — replace all editable fields."""
    data = request.json
    if not data:
        return err('Request body must be JSON')
...
def toggle_product(product_id):
    """POST /api/products/:id/toggle — flip active status."""
    products = load_products()
    for i, p in enumerate(products):
        if p['id'] == product_id:
...
def bulk_action():
    ...
    Body: { "action": "delete" | "activate" | "deactivate", "ids": ["id1", "id2"] }
    """
...
def get_categories():
    """GET /api/categories — unique categories with product counts."""
    products = load_products()
    cats = {}
    for p in products:
